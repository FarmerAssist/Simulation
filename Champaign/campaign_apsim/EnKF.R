...
plot_APSIM_DA <- function(SDA){
  ...
  all %>%
    filter(day<120) %>%
    ggplot(aes(x=day))+
    geom_ribbon(aes(y=Means, ymax=Upper, ymin=Lower, fill=Type), alpha=0.57)+
    geom_line(aes(y=Means, color=Type))+
    geom_point(aes(y=Means, color=Type))+
    #geom_point(aes(y=Obs))+
    geom_pointrange(aes(x=day, ymax=Upper, ymin=Lower, y=Means), data=Obs %>% filter(day<120) )+
    facet_wrap(~Variable)+
    theme(legend.position = "top")->p
  
  ggsave("SDA.png", p, height = 6, width = 10)
